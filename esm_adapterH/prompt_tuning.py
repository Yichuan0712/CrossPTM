...
class PrefixTuning(nn.Module):
  ...
  def __init__(self,
              ...
              num_tasks: int =1,
              task_embedding_dict=None,
              prompt_random_enable = True,
              ):
     ...
     """
      ...
      if prompt_len is not None:
          ...
          for idx in self.prompt_layer_indices:
              print(f"prompt_random_enable: {prompt_random_enable}")
              if prompt_random_enable:  # add randomly intitialized prompt_len to task_embeddings_tensor
                  # print(f'prompt_len: {prompt_len}')
                  embedding_matrices_random = torch.zeros(num_tasks, prompt_len, embed_size)

                  for i in range(num_tasks):
                      embedding_matrices_random[i] = (
                          from_sample_of_embeddings(token_embed_table)([prompt_len, embed_size], i))

                      embedding_matrices = embedding_matrices_random
              else:
                  if task_embedding_dict is not None:
                      # print("print task_embedding_dict in prefix tuning")
                      # print(task_embedding_dict.keys())
                      # print(task_embedding_dict[0].shape)
                      embedding_matrices = torch.stack([task_embedding_dict[i] for i in range(num_tasks)], dim=0)
                  else:
                      embedding_matrices = embedding_matrices_random
  ...
  def _retrieve_embedding(self, task_ids, embeddings):
    ...
    # print(f"_retrieve_embedding length: {len(embeddings)}")
    if len(embeddings) == 1:
      embeddings_expanded = embeddings.expand(len(task_ids), -1, -1)  # Expand along batch size dimension
      # print(f"_retrieve_embedding shape:{embeddings_expanded.shape}") #[batch_size,prompt_len,1280]
      return embeddings_expanded
